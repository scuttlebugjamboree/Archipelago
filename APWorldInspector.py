...
class LabelButGood(MDLabel):
    pass

class OptionsCreator(ThemedApp):
    base_title: str = "Archipelago APWorld Inspector"
    ...
    def create_options_panel(self, world_button: WorldButton):
        self.locations_container.clear_widgets()
        ...
        background = True;
        for location in cls.location_name_to_id.keys():
            if background:
                location_widget = LabelButGood(
                    text=location,
            ...
            expansion_box.layout.add_widget(location_widget)
        self.locations_container.add_widget(expansion_box)

        item_box = ScrollBox()
        item_box.layout.orientation = "vertical"
        item_box.layout.spacing = dp(3)
        item_box.scroll_type = ["bars"]
        item_box.do_scroll_x = False

        background = True;
        for item in cls.item_name_to_id.keys():
            if background:
                item_widget = LabelButGood(
                    text=item,
                    md_bg_color=(1, 1, 1, 0.3),
                    halign="center",
                    padding="3dp",
            ...
            background = not background
            item_box.layout.add_widget(item_widget)
        self.items_container.add_widget(item_box)
    ...
    def build(self):
        self.set_colors()
        self.options = {}
        self.container = Builder.load_file(Utils.local_path("data/apworldinspector.kv"))
        ...
        self.items_container = self.container.ids.items_container
        self.locations_container = self.container.ids.locations_container
